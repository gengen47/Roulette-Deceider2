...
1. **予算フィルタが有効な場合**（上限・下限の指定あり）
    ...
    - **「ランチあり」だが価格不明な場合（救済ロジック）**:
        - `shop.lunch` に「あり」が含まれており、かつ数値が取得できない場合、**ディナー予算から1000円引いた値** を推定ランチ価格として使用します。
        - 例：ディナー予算が「2001～3000円」の場合、推定ランチ価格は「1001円」となり、「上限1500円」の検索にヒットするようになります。
