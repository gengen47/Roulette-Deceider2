...
- **アルゴリズム**:
    1. 対象フィールドのテキストから正規表現 `(\d{1,3}(?:,\d{3})*)` で最初の数値を抽出。
       - 例: "2001～3000円" → `2001`（下限値）
       - 例: "3001～4000円" → `3001`（下限値）
    2. 抽出した数値（予算帯の下限）が、ユーザー指定の予算範囲（`strictMinPrice` 〜 `strictMaxPrice`）に収まっているか判定。
       - **判定のポイント**: 店舗の予算帯の下限（スタート価格）が、ユーザーの上限以下である必要があります。
       - 例1: ユーザー上限3000円 vs 店舗"2001〜3000円" → `2001 <= 3000` (OK)
       - 例2: ユーザー上限3000円 vs 店舗"3001〜4000円" → `3001 <= 3000` (False/除外)
